...
// ../mcp-common/src/osloClient.ts
var OsloClient = class _OsloClient {
  client;
  cookies = "";
  csrfTokenName = "";
  csrfTokenValue = "";
  ...
  async request(url2, method = "GET", data) {
    if (!this.cookies) {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        this.cookies = "";
        return this.request(url2, method, data);
      }
      ...
      logger.error(`Request failed: ${method} ${url2}`, error.message);
      throw error;
    }
  }
  async getSystemInfo() {
  ...
  async getCasterSystemInfo(handleOrPath) {
    const path2 = handleOrPath ? `?folderId=${this.preprocessItemPath(handleOrPath)}` : "";
    return this.request(`/oslo/1.0/rcaster/system${path2}`);
  }
  async getDomains(options = {}) {
    let url2 = "/oslo/1.0/domains/";
    if (options.pageSize) url2 += `?page[size]=${options.pageSize}`;
    return this.request(url2);
  }
  async getMyWorkspace(options = {}) {
    let url2 = "/oslo/1.0/domains/myworkspace";
    if (options.pageSize) url2 += `?page[size]=${options.pageSize}`;
    return this.request(url2);
  }
  async getRecents() {
  ...
  async getReportCasterStatus() {
    return this.request("/oslo/1.0/health/rcaster");
  }
  async testGoogleChatConnection(config) {
...
function activate(context) {
  ...
  let startCommand = vscode.commands.registerCommand("ibi-webfocus.startMcpServer", async () => {
    ...
    const baseUrl = config.get("baseUrl") || "http://localhost:8080/webfocus";
  ...
  let mcpServerDefinitionProvider = vscode.lm.registerMcpServerDefinitionProvider(
    "ibi-webfocus",
    {
      provideMcpServerDefinitions: async () => {
        ...
        return [
          new vscode.McpStdioServerDefinition(
            ...
            {
              OSLO_BASE_URL: config.get("baseUrl") || "http://localhost:8080/webfocus",
              OSLO_USERNAME: config.get("username") || "",
              OSLO_PASSWORD: config.get("password") || ""
            }
          )
        ];
      }
